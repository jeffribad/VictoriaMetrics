...
* BUGFIX: [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/): pick the first backend to process requests when all discovered backends are unavailable. Previously, `vmauth` returned an error without retrying in this case until a health check succeeded for one backend. This fix can increase the success rate. See [#10837](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/10837).
